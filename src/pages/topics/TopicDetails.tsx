...
import { useGetTopic } from "@/lib/api/queries/use-get-topic";
...
export default function TopicDetails() {
  ...
  const [refreshTopic, setRefreshTopic] = useState(0);
  
  const { data: topicData, isLoading: topicLoading, error: topicError, refetch: refetchTopic } = useGetTopic(id!);
  ...
  return (
    <AdminLayout>
      <div className="space-y-6">
          ...
          <CardContent>
            <div className="flex flex-col md:flex-row gap-6 items-start">
              <div className="flex-1 space-y-2">
                <h2 className="text-2xl font-bold">{topicData.data.name}</h2>
                <p className="text-lg text-gray-600">{topicData.data.name_hi}</p>
                <div className="flex items-center gap-4 text-sm text-gray-500 mt-2">
                  <span>Created: {moment(topicData.data.createdAt).format("MMM D, YYYY")}</span>
                  <span>Updated: {moment(topicData.data.updatedAt).format("MMM D, YYYY")}</span>
                </div>
              </div>
            </div>
          </CardContent>
        ...
        <Card>
          <CardHeader>
            <CardTitle>Questions for this Topic</CardTitle>
          </CardHeader>
          <CardContent>
            {questionsLoading ? (
              <div className="flex items-center justify-center py-8">
        ...
        <UpdateTopicModal
          ...
          onSuccess={() => setRefreshTopic((c) => c + 1)}
        />
      </div>
    </AdminLayout>
  );
